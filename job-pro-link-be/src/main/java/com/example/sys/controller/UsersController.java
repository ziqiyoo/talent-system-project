...
//ユーザー名が存在するかを確認する
    ...
    @GetMapping("/user/info/designated")
    public Result<UsersAddReq> getDesignatedUserInfo(@RequestParam("userId") String userId){
        //userIdに基づいてユーザー情報を取得する
        UsersAddReq data=usersService.getDesignatedUserInfo(userId);
        if(data != null){
            return Result.success(data);
        }
        return Result.result(ResultCode.REGISTER_FAILED);
    ...
    @DeleteMapping ("/user/deleted/{userId}")
    public Result<UsersAddReq> deleteUser(@PathVariable("userId") String userId){
        //userIdに基づいてユーザー情報を削除する
