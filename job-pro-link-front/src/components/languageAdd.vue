<template>
    ...
    <el-table :data="languageData" :key="reflush" border :header-cell-style="{ background: '#eef1f4', color: '#606266' }"
        empty-text="データなし">
        <el-table-column prop="languageName" label="語種" width="120" />
...
onMounted(() => {
    emitter.on('languageData', languages);
})
onBeforeMount(() => {
    emitter.off('languageData')
});
const languages: (languages: any) => void = (languages) => {
    for (let i = 0; i < languages.length; i++) {
        languageData.push({
            languageName: languages[i].languageName,
            conversationLevel: languages[i].conversationLevel,
            readingLevel: languages[i].readingLevel,
            credentials: languages[i].credentials
        })
    }
}
...
const handleEdit = (index: number, row: Language) => {
    ...
    languageDialogData.languageName = row.languageName;
    ...
    if (languageDialogData.arrayCreadentials[0].data == "") {
        languageDialogData.arrayCreadentials.splice(0, 1)
    }
}
...
const clearForm = () => {
    ...
    languageDialogData.arrayCreadentials = [{ id: 1, data: "" }]
    setTimeout(() => {
...
const confirm = async () => {
    ...
    if (title.value == "追加言語能力") {
        languageData.push({
            languageName: languageDialogData.languageName,
