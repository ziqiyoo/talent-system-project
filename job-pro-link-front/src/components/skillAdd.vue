...
onMounted(() => {
    emitter.on('skillData', skills);
})

onBeforeMount(() => {
    emitter.off('skillData')
});
const skills: (skills: any) => void = (skills) => {
    for (let i = 0; i < skills.length; i++) {
        skillData.push({
            skillName: skills[i].skillName,
            useDuration: skills[i].useDuration,
            level: skills[i].level,
            credentials: skills[i].credentials
        })
    }
}
...
const handleEdit = (index: number, row: Skill) => {
    ...
    skillDialogData.skillName = row.skillName;
    ...
    if (skillDialogData.arrayCreadentials[0].data == "") {
        skillDialogData.arrayCreadentials.splice(0, 1)
    }
}
...
const clearForm = () => {
    ...
    skillDialogData.arrayCreadentials = [{ id: 1, data: "" }]
    setTimeout(() => {
...
const confirm = async () => {
    ...
    if (title.value == "追加スキル") {
        skillData.push({
            skillName: skillDialogData.skillName,
