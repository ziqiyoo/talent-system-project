...
onMounted(() => {
    emitter.on('projectData', projects);
})
onBeforeMount(() => {
    emitter.off('projectData')
});
const projects: (projects: any) => void = (projects) => {
...
const handleEdit = (index: number, row: Project) => {
    ...
    if (projectDialogData.URLList[0].url == "") {
        projectDialogData.URLList.splice(0, 1)
    }
}
...
const clearForm = () => {
    ...
    projectDialogData.URLList = [{ id: 1, url: "" }]
    setTimeout(() => {
