<template>
    <div class="box" id="userInfo">
        ...
        <div>
            <table class="basicInformation">
                <tr>
                    <td class="firstTd"><span class="label">ユーザー名</span></td>
    ...
    <div style="margin-left: 78%;margin-top: 40px;">
        ...
        <el-button type="primary" @click="deletedUser" size="large" style="margin-left: 30px;font-size: 16px">削除</el-button>
    </div>
</template>
...
onMounted(async () => {
    ...
    if (result.data.workHistories !== null) {
        for (let i = 0; i < result.data.workHistories.length; i++) {
            workData.push({
...
let updateUser = () => {
    $router.push({ name: "talentUpdate", params: { userId: userId } });
    setTimeout(() => {
        emitter.emit('sendUserData', userData.data)
        if (userData.data.skills !== null) { emitter.emit('skillData', userData.data.skills) }
        if (userData.data.languages !== null) { emitter.emit('languageData', userData.data.languages) }
        if (userData.data.courses !== null) { emitter.emit('courseData', userData.data.courses) }
        if (workData !== null) { emitter.emit('careerData', workData) }
        if (projectData !== null) { emitter.emit('projectData', projectData) }
    }, 300);
}
...
let deletedUser = () => {
    ElMessageBox.confirm(
        ...
        const result = await useStore.deletedUser(userId);
        if (result.code == "TAL200") {
            $router.push("/talent/Inquiry")
            ElMessage({
                type: 'success',
                message: '削除完了',
            })
