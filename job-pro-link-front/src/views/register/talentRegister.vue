                ...
                <el-col :span="6" :xs="24" class="right">
                    <el-row>
                        <el-form-item prop="password" label="パスワード" class="rightForm label">
                            <el-input type="password" v-model="talentForm.password" show-password
                ...
                <el-col :span="6" :xs="24">
                    <el-form-item label="写真添付" style="padding-left:30px;" class="label">
                        <el-upload class="avatar-uploader" :show-file-list="false" :auto-upload="false"
                            :on-change="handleChange" :key="talentForm.icon">
...
let disabled = false
let isShow = false
if (userId == null) { disabled = false } else { disabled = true }
...
const showUserData: (userData: any) => void = (userData) => {
    if (userData.icon !== null) {
        talentForm.icon = userData.icon
        isShow = true
    }
    talentForm.userName = userData.userName
    talentForm.mobilePhone = userData.mobilePhone
    talentForm.countryKbn = userData.countryKbn
    talentForm.birthday = userData.birthday
    talentForm.genderKbn = userData.genderKbn
    talentForm.email = userData.email
    talentForm.address = userData.address
    talentForm.educationKbn = userData.educationKbn
    talentForm.specialty = userData.specialty
    talentForm.school = userData.school
    talentForm.admissionDate = userData.admissionDate
    talentForm.graduationDate = userData.graduationDate
    talentForm.description = userData.description
    talentForm.skills = userData.skills
    talentForm.languages = userData.languages
    talentForm.projectHistories = userData.projectHistories
    talentForm.workHistories = userData.workHistories
    talentForm.courses = userData.courses
}
//写真
const handleChange: UploadProps['onChange'] = (uploadFile) => {
    ...
    if (suffName !== 'jpg') {
        ...
        ElMessage.error('写真のサイズは2MBまで！')
        isShow = false
    } else {
        isShow = true
        if (isShow) {
            talentForm.icon = URL.createObjectURL(uploadFile.raw!)
            console.log(talentForm.icon)
        }
    }
}
//メールアドレスをチャックする
