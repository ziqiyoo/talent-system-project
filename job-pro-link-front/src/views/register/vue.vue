    ...
    <div class="talentRegister">
        <el-form :model="talentForm" :rules="rules" ref="talentForms" label-position="top">
                ...
                <el-col :span="6" :xs="24">
                    <el-row>
                        <el-form-item prop="userName" label="ユーザー名(ID)" class="leftForm label">
                            <el-input v-model="talentForm.data.userName"></el-input>
                        </el-form-item>
                    </el-row>
                    <el-row>
                        <el-form-item prop="mobilePhone" label="電話番号" class="leftForm label">
                            <el-input v-model="talentForm.data.mobilePhone"></el-input>
                        </el-form-item>
                    </el-row>
                    <el-row>
                        <el-form-item prop="birthday" label="生年月日" class="label">
                            <el-date-picker v-model="talentForm.data.birthday" type="date" value-format="YYYY-MM-DD"
                ...
                <el-col :span="6" :xs="24" class="right">
                    <el-row>
                        <el-form-item prop="password" label="パスワード" class="rightForm label">
                            <el-input type="password" v-model="talentForm.data.password" show-password></el-input>
                        </el-form-item>
                    </el-row>
                    <el-row>
                        <el-form-item prop="countryKbn" label="国籍" class="rightForm label">
                            <el-select v-model="talentForm.data.countryKbn" class="m-1 country">
                    ...
                    <el-row>
                        <el-form-item prop="genderKbn" label="性別" class="gender label">
                            <el-radio-group v-model="talentForm.data.genderKbn">
                ...
                <el-col :span="6" :xs="24">
                    <el-form-item label="写真添付" style="padding-left:30px;" class="label">
                        <el-upload class="avatar-uploader" :show-file-list="false" :auto-upload="false"
                            ...
                            <img v-if="isShow" :src="talentForm.data.icon" class="avatar" />
            ...
            <el-row>
                <el-col :span="6">
                    <el-form-item prop="email" label="メールアドレス" class="label" style=" width: 59%;">
                        <el-input v-model="talentForm.data.email"></el-input>
                    </el-form-item>
                </el-col>
                <el-col :span="7">
                    <el-form-item style=" width: 70%; " label="住所/アドレス" class="label">
                        <el-input v-model="talentForm.data.address"></el-input>
                    </el-form-item>
                </el-col>
            </el-row>
            ...
            <el-row>
                <el-col :span="5" class="leftForm">
                    ...
                    <el-form-item label="学位" class="label" style="padding-top: 5px;width: 66%;padding-bottom: 10px;">
                        <el-select v-model="talentForm.data.educationKbn" class="m-1 country" placeholder="学位">
                    ...
                    <el-form-item label="専攻" class="label" style="width:66%;">
                        <el-input v-model="talentForm.data.specialty"></el-input>
                    </el-form-item>
                </el-col>
                ...
                <el-col :span="10" style="padding: 96px 0px 0px 50px;">
                    <el-row>
                        <el-form-item label="学校" class="label" style="width:44%;">
                            <el-input v-model="talentForm.data.school"></el-input>
                        </el-form-item>
                    </el-row>

                    <el-row>
                        <el-form-item label="在学期間" class="label" style="margin-top: 10px;">
                            <el-date-picker v-model="talentForm.data.admissionDate" type="date"
                        ...
                        <el-form-item class="label" style="margin-top: 45px;">
                            <el-date-picker v-model="talentForm.data.graduationDate" type="date"
                                :disabled-date="disabledDateEnd" value-format="YYYY-MM-DD" placeholder="卒業年月日" clearable />
                        </el-form-item>
                    </el-row>
                </el-col>
            </el-row>
            ...
            <el-form-item label="自己PR" class="label" style="width: 60%;">
                <el-input v-model="talentForm.data.description" type="textarea" />
            </el-form-item>
...
const disabledDateStart = (time: Date) => {
    let endDateVal = talentForm.data.graduationDate;
    if (endDateVal) {
...
const disabledDateEnd = (time: Date) => {
    let beginDateVal = talentForm.data.admissionDate;
    if (beginDateVal) {
        return (time.getTime() < new Date(beginDateVal).getTime() + 1);
    }
}
watch(() => talentForm.data.userName, (oldvalue, newvalue) => {
...
onMounted(() => {
    ...
    emitter.on('sendUserData', showUserData);
});
onBeforeMount(() => {
    emitter.all.clear()
});
//サブコンポーネントからのデータの受け入れ
const showSkill: (skillData: any) => void = (skillData) => { talentForm.data.skills = skillData }
const showLanguage: (languageData: any) => void = (languageData) => { talentForm.data.languages = languageData }
const showCareer: (careerData: any) => void = (careerData) => { talentForm.data.workHistories = careerData }
const showProject: (projectData: any) => void = (projectData) => { talentForm.data.projectHistories = projectData }
const showCourse: (courseData: any) => void = (courseData) => { talentForm.data.courses = courseData }
const showUserData: (userData: any) => void = (userData) => {
    talentForm.data = userData;
    console.log(userData)
    console.log(talentForm.data)
}
//写真
let isShow = false
const handleChange: UploadProps['onChange'] = (uploadFile) => {
    ...
    if (suffName !== 'jpg') {
        ...
        isShow = true
        if (isShow) { talentForm.data.icon = URL.createObjectURL(uploadFile.raw!) }
    }
}
//メールアドレスをチャックする
...
const validateUserName = async (rule: any, value: any, callback: any) => {
    console.log(rule)
    if (value != "") {
        const result = await useStore.nameRule(talentForm.data.userName);
...
const validatePassword = async (rule: any, value: any, callback: any) => {
    console.log(rule)
    if (value != "") {
        const result = await useStore.passwordRule(talentForm.data.password);
...
let register = async () => {
    try {
        ...
        const paramData = dealObjectValue(talentForm.data)
        console.log(paramData)
        //戻り値の取得
        const registResult = await useStore.talentLogin(paramData);
        console.log("userId:" + registResult.data);
        $router.push({ name: "talentShowcase", params: { userId: registResult.data } });
        ElNotification({
            type: 'success',
